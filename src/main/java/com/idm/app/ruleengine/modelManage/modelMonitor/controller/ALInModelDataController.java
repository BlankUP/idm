...
import com.idm.app.ruleengine.modelManage.modelMonitor.mapper.VAlRuleIndexDataMapper;
...
/**
 * @Author lifadeng
 * @Description: 贷后模型数据监控
 * @Create: 2018/11/6 15:11
 * @Version: 1.0
 */
@Controller
public class ALInModelDataController {

    @Resource
    VAlRuleIndexDataMapper vAlRuleIndexDataMapper;

    private Logger logger = Logger.getLogger(ALInModelDataController.class);

    @RequestMapping("/al_in_model_data_page")
    public String indexManage(Model model){

        return "modelManage/modelMonitor/al_data";
    }

    @RequestMapping("/al_in_model_data")
    @ResponseBody
    public Object indexService(String querySeqNo,String custName, String certNum,String prodId, String modleName, String page,String rows, HttpServletRequest request){
        ...
        try{
            ...
            VAlRuleIndexDataExample example = new VAlRuleIndexDataExample();
            VAlRuleIndexDataExample.Criteria criteria = example.createCriteria();
            ...
            example.or(criteria);
            List<VAlRuleIndexData> data = vAlRuleIndexDataMapper.selectByExample(example);
