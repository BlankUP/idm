...
function doSearch(){
	...
	$("#grid-table").jqGrid('setGridParam', {
	    page : 1,
	    url:root+'/mqi_adbVersion_search.do?versionNo='+str,
	    datatype : "json"
	}).trigger("reloadGrid");
}
...
$(document).ready(function() {
	...
	jQuery(grid_selector).jqGrid({
	    ...
	    page : 1,
	    url:root+'/mqi_adbVersion_search.do',
	    datatype : "json"
	}).trigger("reloadGrid");
});
...
function deleteVersion(versionNo){
	if(confirm("是否删除选中记录？")){
		$.ajax({
	        type: "post",
	        url: root+'/mqi_delete_adbVersion.do',
	...
	//if(applicationReason != null && applicationReason != ""){
	 ...
	 var data = $('#publishFrom').serialize();
		if(confirm("是否发布/恢复该版本？")){
			$.ajax({
		        type: "post",
		        url: root+'/mqi_publishOrRecover_adbVersion.do',
...
//			}else{
	 ...
	 $.ajax({
	        type: "post",
	        url: root+'/mqi_update_adbVersion.do',
	        data:data, 
	        dataType: "json",
	        success: function(data){
...
function uploadFileSubmit(){
    ...
    $.ajax({
        type: "post",
        url: root+'/mqi_save_adbVersion.do',
        data:data,
        dataType: "json",
        success: function(data){
        	if(data.success == true) {
...
function repeatName () {
	...
	if(versionName != null){
		$.ajax({
			type : "post",
			url : root + '/mqi_repeatversionName.do',
			data : {
