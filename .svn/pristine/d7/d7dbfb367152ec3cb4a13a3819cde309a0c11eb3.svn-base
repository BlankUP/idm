...
$(document).ready(function() {
	...
	jQuery(grid_selector).jqGrid({
	    ...
	    page : 1,
	    url:root+'/mqi_scoreCard_TotalScore_Info_query.do',
	    datatype : "json"
	}).trigger("reloadGrid");
});
...
function openModalWin(type,data){
	...
	if(type=="view"){
		$('#dataTitle').html("总流程编码【"+data.serialNo+"】的评分详情");
		...
		jQuery(grid_selector1).jqGrid({
			...
			regional:'cn',
			height: "auto",
			colNames:[ '总流程编码','业务流水号','姓名','证件类型','身份证号','机构编码','机构名称','产品编码','产品名称','模型编码','模型名称','字段','字段中文','字段初始值','命中区间','得分'],
			colModel:[
						 {
							sortable:false,
							name : 'serialNo',
		    ...
		    url:root+'/mqi_scoreCard_detail_Info_query.do',
		    datatype : "json"
		}).trigger("reloadGrid");
	}
}
