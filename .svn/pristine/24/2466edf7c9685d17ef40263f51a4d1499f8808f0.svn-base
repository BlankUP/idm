...
$(document).ready(function() {
	...
	jQuery(grid_selector).jqGrid({
		...
		regional:'cn',
		height: "auto",
		colNames:[ '交易流水号','业务流水号','客户名称','证件类型','证件号','模型名称','申请日期','申请时间','指标','业务流水号','规则名称','规则结果'],
		colModel:[
				  {
						sortable:false,
						name : 'serialNo',
...
//		                }
	    ...
	    page : 1,
	    url:root+'/reject_rule_Info_query.do',
	    datatype : "json"
	}).trigger("reloadGrid");
});
...
function doSearch(){
	...
	$("#grid-table").jqGrid('setGridParam', {
	    page : 1,
	    postData:{serialNo:serialNo,srcDate:srcDate,modelName:modelName,cifName:cifName},
	    url:root+'/reject_rule_Info_query.do',
	    datatype : "json"
	}).trigger("reloadGrid");
}
...
function openModalWin(type,data){
	...
	if(type=="view"){
		...
		jQuery(grid_selector).jqGrid({
		    ...
		    page : 1,
		    url:root+'/ruleResultSerch.do?businessNo=' + businessNo,
		    datatype : "json"
		}).trigger("reloadGrid");
	}
}
...
function doRuleResult(businessNo){
	...
	$("#grid-table").jqGrid('setGridParam', {
	    ...
	    url:root+'/doRuleResult_query.do',
	    datatype : "json"
	}).trigger("reloadGrid");
}
